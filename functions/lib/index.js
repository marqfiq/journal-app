...
exports.onUserUpdate = exports.processScheduledDeletions = exports.deleteUserAccount = exports.syncSubscription = exports.reactivateSubscription = exports.stripeWebhook = exports.verifyCheckoutSession = exports.createCustomerPortal = exports.createCheckoutSession = void 0;
 ...
 */
exports.createCheckoutSession = functions.runWith({ secrets: [stripeSecretKey] }).https.onCall(async (data, context) => {
    // 1. Authenticate User
    if (!context.auth) {
 ...
 */
exports.createCustomerPortal = functions.runWith({ secrets: [stripeSecretKey] }).https.onCall(async (data, context) => {
    // 1. Authenticate User
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
 ...
 */
exports.verifyCheckoutSession = functions.runWith({ secrets: [stripeSecretKey] }).https.onCall(async (data, context) => {
    // 1. Authenticate User
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
    ...
    try {
        ...
        if (session.subscription && typeof session.subscription !== 'string') {
            ...
            await updateUserSubscription(uid, session.customer, sub);
 ...
 */
exports.stripeWebhook = functions.runWith({ secrets: [stripeSecretKey, stripeWebhookSecret] }).https.onRequest(async (req, res) => {
    var _a;
    ...
    try {
        if (event.type === 'checkout.session.completed') {
            ...
            if (session.mode === 'subscription') {
                ...
                let uid = (_a = session.metadata) === null || _a === void 0 ? void 0 : _a.firebaseUID; // metadata on session
 ...
 */
async function updateUserSubscription(uid, customerId, subscription) {
    const priceId = subscription.items.data[0].price.id;
    const status = subscription.status === 'active' || subscription.status === 'trialing' ? 'active' : subscription.status;
    const currentPeriodEnd = firestore_1.Timestamp.fromMillis(subscription.current_period_end * 1000);
    await db.collection('users').doc(uid).update({
 ...
 */
exports.reactivateSubscription = functions.runWith({ secrets: [stripeSecretKey] }).https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
    const uid = context.auth.uid;
 ...
 */
exports.syncSubscription = functions.runWith({ secrets: [stripeSecretKey] }).https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    }
    const uid = context.auth.uid;
 ...
 * Runs every 24 hours.
 */
exports.processScheduledDeletions = functions.runWith({
    secrets: [stripeSecretKey],
    timeoutSeconds: 540,
    memory: '512MB'
...
async function performUserDeletion(userId) {
    ...
    try {
        ...
        if (!userData && !email) {
            ...
            console.log(`[performUserDeletion] User data found. Stripe Customer ID: ${(userData === null || userData === void 0 ? void 0 : userData.stripeCustomerId) || 'None'}`);
        }
        // ---------------------------------------------------------
 ...
 */
exports.onUserUpdate = functions.runWith({ secrets: [stripeSecretKey] }).firestore
